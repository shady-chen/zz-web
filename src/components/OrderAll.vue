<template>
  <div>
    ...
    background: #015fa3!important;"
        >
          <p style="text-align: left!important;" class="my-p2">
            订单序号：
            <b v-for="item in orderData" style="margin-right: 10px;">{{item.id}}</b>
          </p>
    ...
    background: #00477b!important;">
          ...
          <img v-if="orderData[0].bank_which!='支付宝'" src="../assets/bank.jpg" alt="" width="100px" class="bak-logo">
          <img v-if="orderData[0].bank_which=='支付宝'" src="../assets/alipay.jpg" alt="" width="100px" class="bak-logo">

          <div class="right-b">
            <p class="which-b"
               style="text-align: left!important;">{{orderData[0].sys_bank_which}}&nbsp;{{orderData[0].sys_bank_where?('('+orderData[0].sys_name+')'):''}}</p>
            <p class="which-b"
               style="text-align: left!important;">{{orderData[0].sys_bank_where?orderData[0].sys_bank_where:orderData[0].sys_name}}</p>
            <p class="which-b"
               style="text-align: left!important;">{{orderData[0].sys_bank_num}}</p>
          </div>
      ...
      <div style="text-align: left!important;width: 90%;;overflow: hidden;margin:0 auto;">
        ...
        <textarea name="remark" readonly placeholder="如有需要，请输入订单的备注信息" v-model="remark" id="" cols="20" rows="10"
                  style="width: 85%;height: 40px;font-size: 0.3rem;text-align: left!important;"></textarea>
      </div>
...
<script>
  export default {
    ...
    methods: {
      getOne () {
        this.$http.post(this.$store.state.basePath + '/user/date/getOrder2Hour',
          {
            state: 0,
            status: 1
          },
          ...
          (res) => {
            ...
            if (res.body.status == 200) {
              ...
              {
                this.money += Number(this.orderData[i]['money']);
                this.postIds.push(this.orderData[i]['id']);
                this.remark += this.orderData[i]['id'] + " "
              }
              this.remark += "的订单合并";
            }
          })
      ...
      upload: function () {
        ...
        formData.append('order_id', this.postIds)
        formData.append('remark', this.remark)
        formData.append('money', this.money)
        formData.append('sys_bank_num', this.orderData[0].sys_bank_num)
        formData.append('sys_bank_which', this.orderData[0].sys_bank_which)
        formData.append('sys_bank_where', this.orderData[0].sys_bank_where)
        formData.append('sys_bank_where', this.orderData[0].sys_bank_where)
        formData.append('sys_name', this.orderData[0].sys_name)

        this.$http.post(this.$store.state.basePath + '/user/date/OrderTogether', formData, {
